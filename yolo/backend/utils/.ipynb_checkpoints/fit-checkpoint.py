...
class SaveModel(ModelCheckpoint):
  def __init__(self, filepath, monitor = 'val_loss', verbose = 0, save_best_only = False, save_weights_only = False, mode = 'auto', period = 1, include_optimizer = True):
    super(SaveModel, self).__init__(filepath, monitor, verbose, save_best_only, save_weights_only, mode, period)
        ...
        if current is None:
          ...
          if self.monitor_op(current, self.best):
            ...
            if self.save_weights_only:
              ...
              self.model.save(filepath, overwrite = True, include_optimizer = self.include_optimizer)
          else:
            if self.verbose > 0:
...
def train(model,
    ...
    save_model = SaveModel(saved_folder+'/model_no_opt_{epoch:d}.h5', include_optimizer = False, mode='min', verbose=1, monitor='val_loss',period=1,save_best_only=True)
    callbacks_list = [reduce_lr,save_model]
    try:
...
def save_tflite(model):
    ...
    #    print(n.name) 
    model.save("cat/model.h5",include_optimizer=False)
#     converter = tf.lite.TFLiteConverter.from_keras_model_file("model.h5",output_arrays=['detection_layer_35/BiasAdd'])
